...
class Details extends Component {
  constructor(props) {
    ...
    this.tenderingChange = this.props.tenderingChange;
  ...
  confirmVoid = () => {
    alert_double_service.showAlertDouble(
      'Confirm',
      `Are you sure you would like to refund this transaction?`,
      this.refundAction
    );
  };
  getSettingOTP = async () => {
  ...
  refundAction = () => {
    const response =
      this.props.transaction.process || this.props.transaction.process.response;
    // const response = process.response;
        ...
        let handle = alertmsg => {
          ...
          if (Platform.OS == 'ios') {
            this.setState({ loading: false }, () => {
              this.setState(
                { alertActive: true, alertMessage: alertmsg },
                () => {
                  //NavigationService.reset('CashRegister');
                }
              );
            ...
            /*alert_service.showAlert(alertmsg, () => {
            NavigationService.reset('CashRegister');
          });*/
          }
        };
  ...
  render() {
    const { title, icon, items, total } = this.props;
            ...
            <View style={styles.footerContainer}>
              {this.tenderingChange > 0 && (
                ...
                >
                  ...
                  >
                    ₹{this.tenderingChange.toFixed(2)}
                  </TextMontserrat>
                </View>
              )}
              <View
