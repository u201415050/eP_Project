...
class ProductDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      (product.cgst !== '' && parseFloat(product.cgst) != '0.0') ||
      (product.igst !== '' && parseFloat(product.igst) != '0.0')
    ) {
      this.setState({ taxFromSavedContainerHeight: hp('24.5%') });
    } else if (product.vat !== '' && parseFloat(product.vat) != '0.0') {
      this.setState({ taxFromSavedContainerHeight: hp('10.4%') });
    }
  }
  ...
  render() {
    ...
    //console.log('INDICE', this.props.index)
    return (
      <View
        style={{
          alignItems: 'center',
              ...
              <View style={styles.container}>
                ...
                >
                  ...
                  {formatNumberCommasDecimal(
                    parseFloat(quant * unitPrice).toFixed(2)
                  )}
                </Text>
              </View>
              {discount > 0 ? (
