...
import { getCreateRow, createRow } from "../../services/realm_service";
...
export default class PaymentService extends EventEmitter {
    ...
    if (offline) {
      ...
      requests.forEach(request => {
        ...
        try {
          ...
          //alert(JSON.stringify(itemToPay))
  ...
  async paymentFinalize(paymentId: number): Promise<Finalize> {
    try {
      ...
      if (res.success === 0 && res.message.indexOf("Approved") == -1) {
        throw new Error(res.message);
      }
      this.emit("done");
      return {
  ...
  async cancel(paymentId: number) {
    try {
      //this.emit("request");
      this.emit("cancelled");
      return epaisaRequest(paymentId, "/payment/cancel", "POST").then(res => {
        if (res.success === 0) {
          this.emit("done");
          console.log(res);
